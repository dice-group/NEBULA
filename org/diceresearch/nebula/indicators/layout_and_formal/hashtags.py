...
def _extract_hash_tags_unique(input_text: str) -> List[str]:
    return list(set(hashtag[1:] for hashtag in input_text.split() if hashtag.startswith("#")))


def detect_excessive_hashtag_usage(input_text, config: tomlkit.TOMLDocument) \
        -> Union[bool, Dict[str, Union[str, List[str]]]]:
    hashtag_count_threshold, display_message_warning = _load_settings_from_config(config)

    hashtag_list = _extract_hash_tags_unique(input_text)
    hashtag_count = len(hashtag_list)
    if hashtag_list and hashtag_count >= hashtag_count_threshold:
        results = {"display_message": display_message_warning, "hashtags_list": hashtag_list}
        logging.info(display_message_warning)
        logging.info(f"Total number of detected hashtags (without duplicates): {hashtag_count}")
