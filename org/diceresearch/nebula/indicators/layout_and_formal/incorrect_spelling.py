...
def _load_settings_from_config(config: tomlkit.TOMLDocument):
    try:
        ...
        logging.error(
            "Incorrect spelling configuration! Check that all required settings are defined."
        )
...
def check_incorrect_spelling(
    ...
    if misspelled_words_start_end:
        results = {
            ...
            "misspelled_word_positions": [
                (word[1], word[2]) for word in misspelled_words_start_end
            ],
        }
        ...
        for misspelled_word_start_end in misspelled_words_start_end:
            logging.info(
                f"Word: {misspelled_word_start_end[0]} "
